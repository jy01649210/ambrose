...
public class HiveJob extends Job {
  ...
  public void setJobStats(Map<String, Double> counterNameToValue, int totalMappers,
    ...
    metrics.put("avgMapTime", getAvgCounterValue(counterNameToValue, MetricsCounter.SLOTS_MILLIS_MAPS, totalMappers));
    metrics.put("avgReduceTime", getAvgCounterValue(counterNameToValue, MetricsCounter.SLOTS_MILLIS_REDUCES, totalReducers));
    metrics.put("bytesWritten",
        getCounterValue(counterNameToValue, MetricsCounter.FILE_BYTES_WRITTEN));
    metrics.put("hdfsBytesWritten", getCounterValue(counterNameToValue, MetricsCounter.HDFS_BYTES_WRITTEN));
  ...
  private Double getCounterValue(Map<String, Double> counterNameToValue, MetricsCounter hjc) {
    ...
    if(counterNameToValue.get(keys[0]) == null && counterNameToValue.get(keys[1]) == null) {
